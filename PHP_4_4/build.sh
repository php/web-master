...
if test "$DO_CVS_CLEAN" = "1"; then
	cd $PHP_SRC
	./cvsclean || exit 1
	rm -f ext/standard/var_unserializer.c
	rm -f ext/standard/url_scanner_ex.c
	cvs up
...
#	make ext/standard/parsedate.c
#	cat ext/standard/parsedate.c | sed 's/^#line.*y\.tab\.c.*$//' > ext/standard/parsedate.new
#	mv -f ext/standard/parsedate.new ext/standard/parsedate.c
	make || exit 1
	make install || exit 1
fi
