...
if (@mysql_pconnect("localhost","nobody","")) {
  if (@mysql_select_db("php3")) {
    $res = @mysql_query("SELECT mirrors.*,country.name AS cname,(DATE_SUB(NOW(),INTERVAL 3 DAY) < mirrors.lastupdated) AS current FROM mirrors LEFT JOIN country ON mirrors.cc = country.id ORDER BY country.name,hostname");
    if ($res) {
      echo "<?php\n\$MIRRORS = array(\n";
      while ($row = @mysql_fetch_array($res)) {
        ...
        if (!$row["active"] || !$row["current"]) { $row["mirrortype"] = 2; }
